...
document.getElementById('saveBtn').addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/saveLink', {
      ...
      body: JSON.stringify({ url, title, originClient: 'web' })
    });
...
document.getElementById('viewBtn').addEventListener('click', async () => {
  try {
    ...
    links.forEach(link => {
      ...
      // Edit button (fixed autofill)
      const editBtn = document.createElement('button');
      editBtn.textContent = 'Edit';
      editBtn.onclick = async () => {
        const newTitle = prompt('Enter new title:', link.title); // autofills with title
        const newUrl = prompt('Enter new URL:', link.url);       // autofills with URL
